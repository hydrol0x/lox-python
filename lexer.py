...
class Lexer:
    ...
    def match_next(self, character):
        ...
        if self.source[self.current] != character: return False 
    ...
    def string(self):
        ...
        if self.is_at_end():
            report(self.line, f"{self.start}-{self.current}", "Unterminated string.")
            return
    ...
    def multi_comment(self):
        ...
        while counter != 0 and not self.is_at_end():
            ...
            self.advance()
        if counter != 0:
            report(first_comment_line, f"{self.start}-{self.current}", "Unterminated multiline comment.")
            return
        ...
        match (char):
            ...
            case _:
                if char.isnumeric():
                    ...
                    report(self.line, f"{self.start}-{self.current}", f"Unexpected character {char}")
